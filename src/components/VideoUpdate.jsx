...
export default function AlterUploads() {
    ...
    const [formValues, setFormValues] = useState({ title: '', artBy: '', videoUrl: '', content: '', date: '' });
    ...
    useEffect(() => {
        const fetchVideos = async () => {
            try {
                const q = query(collection(db, 'art-video'), orderBy('date', 'desc'));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await updateDoc(doc(db, 'art-video', editingVideo), {
                ...formValues,
    ...
    const handleDelete = async (videoId) => {
        try {
            await deleteDoc(doc(db, 'art-video', videoId));
            ...
            <div className="w-full max-w-2xl mt-8 bg-white p-6 rounded-lg shadow-md">
                {currentVideos.map(video => (
                    <div key={video.id} className="mb-4 p-4 border-b border-gray-300 last:border-b-0 gradient-background1 rounded-lg">
                        ...
                        <p className=" text-white">Art By: {video.artBy}</p>
                ...
                <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center">
                    <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-lg mx-auto">
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="mb-4">
                                <label className="block text-gray-700 font-semibold mb-2">Art By:</label>
                                <input
                                    type="text"
                                    name="artBy"
                                    value={formValues.artBy}
                                    onChange={handleChange}
                                    required
                                    className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                />
